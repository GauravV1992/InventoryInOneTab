/* Sales order — Terms & Conditions stored per sale (no master) */
USE PawanPutra;
GO

IF COL_LENGTH('dbo.SalesHeader', 'TermsAndConditions') IS NULL
    ALTER TABLE dbo.SalesHeader ADD TermsAndConditions NVARCHAR(MAX) NULL;
GO
...
BEGIN
    ...
    SELECT
        ...
        h.CustomerName, h.Remark, h.TermsAndConditions, h.CreatedAt,
...
BEGIN
    ...
    BEGIN TRY
        ...
        IF OBJECT_ID('dbo.StockFifoBatch', 'U') IS NOT NULL
        BEGIN
        ...
        IF OBJECT_ID('dbo.sp_FifoConsumeForSales', 'P') IS NOT NULL
...
BEGIN
    ...
    BEGIN TRY
        ...
        IF OBJECT_ID('dbo.sp_FifoRestoreBySales', 'P') IS NOT NULL
...
PRINT N'Sales TermsAndConditions column and procedures applied.';
GO
